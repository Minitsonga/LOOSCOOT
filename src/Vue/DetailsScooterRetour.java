...
public class DetailsScooterRetour extends JPanel {


    public DetailsScooterRetour(Parc parc, Scooter scooter, LocalDate startDate, LocalDate endDate, Client client, JFrame frame) {
        ...
        JLabel KMTitleTxt = new JLabel("Indiquez le nombre de kilometres effectues : ");
        KMTitleTxt.setFont(new Font("Tahoma", Font.BOLD, 16));
        JTextField kmTxt = new JTextField("    KM    ");
        kmTxt.addFocusListener(new FocusListener() {
            @Override
            public void focusGained(FocusEvent e) {
                if (kmTxt.getText().equals("    KM    ")) {
                    kmTxt.setText("");
                    kmTxt.setForeground(Color.BLACK);
                }
            }

            @Override
            public void focusLost(FocusEvent e) {
                if (kmTxt.getText().isEmpty()) {
                    kmTxt.setForeground(Color.GRAY);
                    kmTxt.setText("    KM    ");
                }
            }
        });
        ...
        panelKM.add(KMTitleTxt, gbc);
        panelKM.add(kmTxt, gbc);
        ...
        returnButton.setMinimumSize(new Dimension(100, 20));
        returnButton.setPreferredSize(new Dimension(150, 40));
        returnButton.setFont(new Font("Tahoma", Font.PLAIN, 20));

        ValidateLoc_Return locationWindow = new ValidateLoc_Return(parc, scooter, startDate, endDate, client, frame, Parc.ChoiceClient.retour, kmTxt, errorTxt);
